...
| **PDF locked** (the $13 wall) | set `STRIPE_SECRET_KEY=sk_test_…` so `pdf_billing` is on → a finished plan on BYOK offers **🔓 Unlock clean PDF, $13** (watermarked copy free); raw `.zip` stays free |
...
- **Stripe ($13 per-plan unlock + Pro $29/mo, BUILT):** Stripe **test mode** keys (`STRIPE_SECRET_KEY=sk_test_…`,
  ...
  $13 and $29 prices are built inline). Or skip Stripe and grant a comp directly: `dev.py buy <email>`.
...
- **$13 purchase + subscribe (go-live):** do one real purchase of each to yourself and refund them in Stripe.
